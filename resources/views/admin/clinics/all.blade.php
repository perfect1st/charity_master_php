...
<div class="iframe-container"   dir="{{App::getLocale()=='ar' ? 'rtl' : 'ltr'}}" style="{{App::getLocale()=='ar' ? 'margin-right: 3%' : 'margin-left: 3%'}} ">
    <h1>  {{  App::getLocale()=='ar' ? $dep->department_title_ar : $dep->department_title_en }} </h1>
    <div style="display: flex">
        <div style="width: 82%">
            <a class="btn btn-success" href="{{ route('addClinicPage', $dep->id) }}" >
    ...
    <div style="margin-top: 10px;">
        <form  method="GET" action="{{route('searcharticlesClinic')}}">
            <input type="hidden" name="id" value="{{$dep->id}}">
            <div style="display: flex;width: 37%;margin-bottom: 2%;{{ App::getLocale()=='ar' ? 'float: left' : 'float: right'}};">
                ...
                <select style="{{App::getLocale()=='ar' ? 'margin-right: 2%' : 'margin-left: 2%'}}" name="type"  required>
                    ...
                    <option value="articles_isactive">
                        ...
                        <option value="price">
                            {{App::getLocale()=='ar' ? 'السعر' : 'price'}}</option>
                </select>
    ...
    <form method="get" id="myForm" action="{{route('sortArticleClinic')}}">
        @csrf
        <input type="hidden" name="id" value="{{$dep->id}}">
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th scope="col"  onclick="field(event)"> {{App::getLocale()=='ar' ? 'السعر' :'price' }}
                </th>
                <th scope="col">
                ...
                <tr>
                    ...
                    <td>
                        <a class="btn btn-warning" style="margin-top: 10px;"
                            href="{{ route('editClinicPage',[$slider->id,$dep->id]) }}">
